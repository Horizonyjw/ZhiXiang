"""
读取 radar_v1 handoff_samples.npz 并做 U-Net 真实前向测试。

用法（仓库根目录）:
  .\\.venv\\Scripts\\python.exe -m train.test_handoff_forward
  .\\.venv\\Scripts\\python.exe -m train.test_handoff_forward --config configs/20260812-unet-radar_v1-01.yaml
"""
...
def run(cfg: dict) -> Path:
    ...
    if "metadata_json" in payload:
        # 保留前 batch 条原始 metadata 字符串
        raw = payload["metadata_json"]
        try:
            meta["sample_metadata"] = [str(raw[i]) for i in range(batch)]
        except Exception:
            pass
    ...
    summary = {
        ...
        "forward_ok": True,
        "predictions_untrained": str(pred_path.relative_to(repo_root())),
    }
    summary_path = out_dir / "forward_summary.json"
    summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    print("saved:", pred_path)
    print("handoff forward OK")
    return pred_path


def main() -> None:
    parser = argparse.ArgumentParser(description="radar_v1 handoff U-Net 前向测试")
